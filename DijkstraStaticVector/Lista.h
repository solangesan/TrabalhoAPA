...
using std::cout;
using std::endl;
namespace autoreferencia {
  // Para utilizar a classe Lista<T> o tipo de dado fornecido no
  // lugar do parâmetro de tipo T deve possuir um construtor de
  // cópia e os operadores <<, ==, !=, e = sobrecarregados.
	...
	template <class T> void Lista<T>::insere (const T& x) {
	...
	// Insere antes do primeiro item da lista
	template <class T> void Lista<T>::inserePrimeiro (T& item) {
	...
	template <class T> T *Lista<T>::retira (const T& chave) throw ( logic_error ) {
        ...
        q->item = 0; // transfere a posse da mem\'oria
